// mcpServersRepo.ts — хранение конфигов MCP-серверов (порт из manager-agent).
// Источник правды — админка; секреты (в args/env) наружу не отдаются
// (см. routes.admin — маскирование).
...
export class McpServersRepo {
  ...
  count(): number {
    return (this.db.prepare(`SELECT COUNT(*) AS c FROM mcp_servers`).get() as { c: number }).c;
  }

  /** Полностью заменяет список (админка — источник правды). */
