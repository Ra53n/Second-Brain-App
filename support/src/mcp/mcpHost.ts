// mcpHost.ts — generic MCP-хост (порт runner/mcpHost.ts из manager-agent).
// Подключается к N MCP-серверам (stdio), агрегирует инструменты, квалифицирует
// имена `<slug>__<tool>` и маршрутизирует вызовы. Ошибки инструментов
// возвращаются строкой "ERROR: …", чтобы tool-loop мог отреагировать.
...
export const defaultConnector: McpConnector = async (server) => {
  ...
  const client = new Client({ name: "support-assistant", version: "0.1.0" }, { capabilities: {} });
...
export class McpHostImpl implements McpHost {
  ...
  private route = new Map<string, { serverId: string; tool: string }>();
  ...
  async refresh(servers: McpServerConfig[]): Promise<void> {
    ...
    // Закрыть удалённые/выключенные/изменённые.
