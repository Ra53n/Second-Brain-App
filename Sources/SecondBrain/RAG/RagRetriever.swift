...
enum RagRetriever {
    ...
    /// ГРАНИЦЕ чанка, не внутри. nil — попаданий нет / всё пустое. `secure: false`
    /// (задача 101) — заголовок без anti-injection оговорки.
    static func buildBlock(hits: [RagHit], budgetTokens: Int, secure: Bool = true) -> String? {
        ...
        let header = secure
            ? "Фрагменты базы знаний (метка: [источник: [[заметка]] · раздел]). "
