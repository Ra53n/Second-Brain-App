...
начинаем только после Ф2. Фазы = стадии конвейера (см. «Конвейер разработки»): для
содержательной задачи техлид делегирует их субагентам, каждая оставляет свой артефакт.

**Ф1. Интервью — стадия `product-analyst`.** Веди себя как senior/staff-инженер, которому
принесли запрос: пока не поймёшь сценарий и границы, не проектируй. Обязательный чек-лист
вопросов пользователю (в автономном режиме, когда вопросы запрещены, — ответы на них
документируются как допущения в `## Допущения и corner-кейсы`, не спрашиваются):
...
**Ф2. Архитектурный разбор — стадия `architect`** (артефакт `## Архитектура`). Наложи
задачу на существующую систему:
...
Содержательная задача проходит конвейер ролей, как в компании. **Главный агент — техлид: он
оркестрирует и сам код не пишет**, а делегирует каждую стадию своему субагенту. Пропуск
стадии виден в диффе: у каждой — обязательный артефакт-секция в файле задачи, без неё задача
не готова.

| Стадия | Субагент | Обязательный артефакт |
|---|---|---|
| Продукт | `product-analyst` | `## Допущения и corner-кейсы` (автономный режим — документирует, не спрашивает) |
| Архитектура | `architect` | `## Архитектура` (размещение, что переиспользовать, инварианты, где чистое ядро) |
...
- **Ревью-гейт:** без вердикта `GO` от `reviewer` коммита нет. NO-GO → техлид возвращает
  задачу на стадию с проблемой.
- **Порог.** Полный конвейер — на всё, что требует файла задачи (новый тип, поведение, данные
  пользователя). Тривиальное (опечатка, правка доков, однострочник без смены поведения) —
  техлид делает solo, но верификация обязательна и там.
- **Результат стадии техлид не переделывает и не перепроверяет вручную** — проверяет
  `reviewer` как отдельная стадия. Либо конвейер, либо (для тривиального) solo — среднего нет.
...
- **Содержательная задача идёт через конвейер ролей** (см. «Конвейер разработки»): техлид
  делегирует стадии субагентам и сам не кодит; каждая стадия оставляет артефакт в файле
  задачи; без вердикта `GO` от `reviewer` коммита нет. Solo — только тривиальное.
...
7. **Ответ пользователю — 3–5 строк:** что сделано, что проверено, что требует решения.
