...
- **После задачи/PR — цикл прогона по импакту** (`/smoke-cycle`, `scripts/smoke-all.sh`): затронутые
  код-тесты + смоук-сценарии выбираются по `smoke/impact-map.tsv`, единый отчёт с HTML и
  локализацией падения; под новую фичу дописывается смоук-сценарий. Уровни 1/2 — задачи 58/59, петля — 60.

### Сценарные профили

Рядом с ролями конвейера есть три профиля под **форму задачи** (`.claude/agents/`, что делает
каждый — в его `description`): `research`, `bugfix`, `refactor`. `research` read-only и
терминальный: дифа не производит, гейт ему не нужен. `bugfix`/`refactor` встают на стадию
«Разработка» вместо `coder` и **не коммитят** — дальше всё как обычно: `tester`, `reviewer`,
ревью-гейт, коммит за техлидом. Профиль выбирается по форме запроса, а не вместо конвейера.
...
- **После правки/PR — прогон по импакту**: `./scripts/smoke-all.sh` по `smoke/impact-map.tsv`
  сам выбирает затронутые код-тесты и смоук-сценарии, даёт единый отчёт + HTML и при падении
  подсказывает место. Цикл целиком и дописывание сценариев под новую фичу — `/smoke-cycle`.
