...
Tools/        — встроенные инструменты чата: BuiltinTool/ToolRegistry/ToolExecutor, git-обзор проекта
...
**Встроенные инструменты проекта** (задачи 21, 25): пользователь выбирает git-репозиторий в настройках (`projectRepoPath`; репо самого проекта, репо vault или произвольная папка) → per-чат тумблер «Инструменты проекта» в меню инструментов → модель получает read-only набор git_branches/git_status/git_log/git_diff/list_files/read_file. `/help` (задача 22) отвечает по документации репозитория: при доступном эмбеддере — RAG-ретрив top-K чанков из `<repo-id>/project-docs.sqlite` (ленивая инкрементальная индексация README+docs при вызове), иначе — фолбэк на полный контекст с бюджетом. Архитектура: протокол `BuiltinTool` (name+description+parameters — для LLM, execute — логика), каждый инструмент — отдельный класс, `ToolRegistry` регистрирует набор, общий `ToolExecutor` (actor) находит тулзу по имени и исполняет; ошибки — текстом `ERROR:` (модель реагирует). Имена без «__» — не пересекаются с qualified-именами MCP (`slug__tool`), маршрутизация вызовов в ChatViewModel по этому признаку. Пути от модели — недоверенный ввод (`SafePath`: нормализация, симлинки, запрет выхода из корня); чтение файлов с капом 64 КБ, бинарники отклоняются. Внешняя альтернатива — шаблон `mcp-server-git` (uvx) в MCP-настройках.
