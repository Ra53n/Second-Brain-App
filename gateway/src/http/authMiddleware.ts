// authMiddleware.ts — admin-доступ по bearer-токену из /etc/llm-gateway.env.
// Чат публичный (за rate-limit), поэтому cookie-сессий здесь нет.
...
export function requireAdmin(ctx: AppContext) {
  return async (req: FastifyRequest, _reply: FastifyReply) => {
    if (!isAdmin(req, ctx)) throw new UnauthorizedError("Нужен admin-токен gateway");
  };
}
