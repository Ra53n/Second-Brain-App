...
DESTRUCTIVE='Записать|Запись|Удалить|Удаление|Push|Отправить|Коммит|Commit|Индексировать|Переиндекс|Перестроить|Синхронизац|Откатить|Запустить тюн|Снять baseline|Сгенерировать критерии|Импортировать|Батч: базовая|Батч: тюн'
