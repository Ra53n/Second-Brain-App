// web/index.ts — две inline-страницы: запуск прогонов (публичная) и админка
// (настройки агента + журнал фаз). String.raw, без сборщика. Порт стиля gateway.
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  header { font-weight:600; font-size:18px; margin-bottom:4px; }
  .muted { color:#9aa4b2; font-size:13px; }
  select, textarea { width:100%; background:#161a22; color:#e6e6e6; border:1px solid #2b3140; border-radius:8px; padding:10px; font:inherit; margin-top:6px; }
  label { display:block; margin-top:14px; color:#9aa4b2; font-size:12px; }
  button { background:#3b82f6; color:#fff; border:0; border-radius:8px; padding:10px 20px; font-weight:600; cursor:pointer; margin-top:14px; }
  button:disabled { opacity:.5; cursor:default; }
  .inline { display:inline-flex; align-items:center; gap:6px; margin-top:14px; }
...
<header>LLM Harness · execution loop</header>
<div class="muted">Генерирует JS-код по задаче через LLM Gateway, гоняет цикл генерация → node --check → security review → коммит. Все вызовы к модели идут через прокси. <a href="/harness/admin">админка →</a></div>

<label>Встроенная задача</label>
<select id="task"><option value="">— свой промпт ниже —</option></select>
<label>Свой промпт (если задача не выбрана)</label>
<textarea id="prompt" rows="3" placeholder="Например: сохрани токен пользователя в файл"></textarea>
<label class="inline"><input type="checkbox" id="secure" checked> защиты включены (secure)</label>
<div><button id="run">Запустить прогон</button></div>

<div id="steps"></div>
<script>
  var $ = function(id){ return document.getElementById(id); };
  function esc(s){ var d=document.createElement('div'); d.textContent=s==null?'':String(s); return d.innerHTML; }
  fetch('/harness/tasks').then(function(r){return r.json();}).then(function(j){
    j.items.forEach(function(t){ var o=document.createElement('option'); o.value=t.id; o.textContent=t.title+' — '+t.trap; $('task').appendChild(o); });
  });
  var poll = null;
  function render(data){
    var run = data.run, steps = data.steps || [];
    var head = '<div class="step"><b>'+esc(run.taskTitle)+'</b><span class="status '+run.status+'">'+run.status+'</span>'+
    ...
    var body = steps.map(function(s){
      ...
      var fh = finds.map(function(f){ return '<div class="find">['+f.severity+'] строка '+(f.line==null?'?':f.line)+': '+esc(f.issue)+'</div>'; }).join('');
      var gwl = (gw.inputAction && gw.inputAction!=='allow') ? '<div class="muted">gateway: '+gw.inputAction+' ['+(gw.findingTypes||[]).join(', ')+']</div>' : '';
      return '<div class="step"><div class="ph">'+esc(s.phase)+' · круг '+s.round+'</div><div class="dp">'+esc(s.display)+'</div>'+fh+gwl+
    ...
    $('steps').innerHTML = head + body + warns;
  }
  function track(id){
    if (poll) clearInterval(poll);
    poll = setInterval(function(){
      fetch('/harness/runs/'+id).then(function(r){return r.json();}).then(function(d){
        render(d);
  ...
  $('run').addEventListener('click', function(){
    var body = { taskId: $('task').value || undefined, prompt: $('prompt').value.trim() || undefined, secure: $('secure').checked };
    $('run').disabled = true;
    fetch('/harness/runs', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) })
      .then(function(r){ return r.json().then(function(j){ return {ok:r.ok,j:j}; }); })
      .then(function(res){ $('run').disabled=false; if(!res.ok){ alert((res.j.error&&res.j.error.message)||'Ошибка'); return; } $('steps').innerHTML='<div class="muted">прогон запущен…</div>'; track(res.j.id); })
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .card span { color:#9aa4b2; font-size:12px; } .card b { font-size:16px; display:block; word-break:break-word; }
...
<h1>LLM Harness · админка</h1>
<label>Admin-токен (Bearer)</label>
<input id="token" placeholder="HARNESS_API_TOKEN" autocomplete="off">
...
<script>
  ...
  function loadCfg(){
    return api('/harness/admin/config').then(function(c){
      $('cfg').innerHTML = card('Версия', c.version)+card('Модель', c.model)+card('Gateway', c.gwUrl)+
        card('maxRounds', c.maxRounds)+card('maxTokens', c.maxTokensHint)+card('secure по умолч.', c.defaultSecure)+
        card('Rate limit/мин', c.rateLimitPerMin)+card('Суточный лимит', c.dailyLimit)+card('Канарейка', c.canaryEnabled?'включена':'нет')+
        card('Задач', c.tasks.length);
      $('sysprompt').textContent = c.systemPromptPreview;
    });
  }
  function loadRuns(){
    return api('/harness/admin/runs?limit=100').then(function(j){
      var tb=$('runs').querySelector('tbody'); tb.innerHTML='';
      if(!j.items.length){ tb.innerHTML='<tr><td colspan="7" class="muted">пусто — запусти прогон на /harness/</td></tr>'; return; }
  ...
  function detail(id){
    api('/harness/admin/runs/'+id+'/steps').then(function(d){
      var steps=d.steps||[];
      var html='<h2>Прогон '+esc(id.slice(0,8))+'</h2>';
      steps.forEach(function(s){
        var finds=JSON.parse(s.findings_json||'[]'); var gw=JSON.parse(s.gateway_json||'{}'); var eg=JSON.parse(s.egress_json||'[]');
        html+='<div class="card"><span>'+esc(s.phase)+' · круг '+s.round+'</span><div>'+esc(s.display)+'</div>';
        finds.forEach(function(f){ html+='<div style="color:#ffb4b4">['+f.severity+'] строка '+(f.line==null?'?':f.line)+': '+esc(f.issue)+'</div>'; });
        ...
        html+='<div class="muted">промпт: '+esc((s.prompt_preview||'').slice(0,160))+'</div>';
        html+='<div class="muted">ответ: '+esc((s.answer_preview||'').slice(0,200))+'</div></div>';
      });
      $('detail').innerHTML=html; $('detail').scrollIntoView({behavior:'smooth'});
    }).catch(function(e){ alert(e.message); });
  }
  function loadAll(){ loadCfg().then(loadRuns).then(function(){ localStorage.setItem('htok',$('token').value.trim()); $('out').style.display='block'; }).catch(function(e){ alert('Ошибка: '+e.message+' — проверь токен'); }); }
  $('load').addEventListener('click', loadAll);
  $('refresh').addEventListener('click', function(){ loadRuns().catch(function(){}); });
  var saved=localStorage.getItem('htok'); if(saved) $('token').value=saved;
</script></div></body></html>`;
...
export function registerWebRoutes(app: FastifyInstance): void {
  ...
  app.get("/harness", html(RUN_PAGE) as never);
  app.get("/harness/", html(RUN_PAGE) as never);
  app.get("/harness/admin", html(ADMIN_PAGE) as never);
}
